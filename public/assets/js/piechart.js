...
async function makeChart() {
  ...
  // function to convert numbers to USD currency with $ sign
  ...
  function reducer(resultObject, member) {
    ...
    member.funds.forEach((fund) => {
      ...
      let gain = '';
      // Gain or loss value ('+' is gain and '-' is loss)
      let gainOrLoss = Math.round(((fund.totalValue - fund.initial_cost) / fund.initial_cost)*100*100) / 100;
      ...
      let fundCreatedAt = fund.createdAt.substr(0, 10);

      resultObject.labels.push(
        `${member.first_name} ${member.last_name}: ${
          fund.quantity
        } ${fund.stock_name.toUpperCase()} ${newValue}` + ` ${gain} of ${gainOrLoss}% since ${fundCreatedAt}`
      );
    });
    return resultObject;
  };
